...
TEST(collaboration_protocol, encodes_and_decodes_text_envelopes) {
	WireEnvelope input{ProtocolVersion, "heartbeat", "request-1", 42, R"({"future_field":true})"};
...
TEST(collaboration_protocol, recognizes_graceful_room_leave) {
	WireEnvelope leave{ProtocolVersion, "leave_room", "leave-1", 42, "{}"};
...
TEST(collaboration_protocol, compresses_large_envelopes_and_round_trips_utf8) {
	...
	WireEnvelope input{ProtocolVersion, "snapshot_state", "request-2", 9, "{\"text\":\"" + text + "\"}"};
	auto frame = EncodeFrame(input);
...
TEST(collaboration_protocol, decodes_json_unicode_escapes_from_go) {
	auto prefix = std::string(R"({"protocol_version":2,"type":"heartbeat","request_id":"unicode","room_revision":0,"payload":{"text":")");
...
TEST(collaboration_protocol, rejects_incompatible_or_malformed_frames) {
	EXPECT_THROW(DecodeEnvelope(R"({"protocol_version":1,"type":"heartbeat","request_id":"r","room_revision":0,"payload":{}})"), std::invalid_argument);
	EXPECT_THROW(DecodeEnvelope(R"({"protocol_version":2,"type":"future_required_message","request_id":"r","room_revision":0,"payload":{}})"), std::invalid_argument);
	EXPECT_THROW(DecodeFrame(true, {0x02, 0x00}), std::invalid_argument);
	EXPECT_THROW(DecodeEnvelope(R"({"protocol_version":2,"type":"heartbeat","request_id":"","room_revision":0,"payload":{}})"), std::invalid_argument);
	WireEnvelope compressed{ProtocolVersion, "heartbeat", "r", 0, "{}"};
